...
final class MemoryPromptSection implements SystemPromptSection {
    ...
    public void appendTo(StringBuilder content, List<String> sourceNames) {
        ...
        content.append("- 接手项目、恢复长期上下文、开始非平凡开发任务、执行计划、处理用户纠正、或进入不熟悉模块前，如果存在 L2 项目记忆，必须优先读取 L2：`<cwd>/.ly-code/memory.md` 或项目根 `MEMORY.md`。\n");
        ...
        content.append("- L0: `~/.ly-code/memory.md` 始终注入。它是 L1 索引和 memory 治理入口，只应包含下层记忆指针、触发场景和少量红线规则。\n");
        content.append("- L1: `~/.ly-code/memory/*` 不自动注入。涉及用户长期偏好、跨项目指导、重要纠错、协作习惯时，先读 L0，再根据 L0 索引按需读取 L1。\n");
        content.append("- L2: `<cwd>/.ly-code/memory.md` 或项目根 `MEMORY.md` 不自动注入。它是项目方向层；接手项目或涉及当前项目目标、边界、设计方向、用户纠错、准则级事实、L3 skill 索引或长期上下文时，必须优先读取它；与项目方向无关时不要读取。\n");
        content.append("- L3: `<cwd>/.ly-code/skills/*` 由 Skills 区块索引。它是项目具体知识和处理技巧层；涉及模块知识、处理流程、排障技巧、实现取舍、验证方式或已沉淀操作经验时，按 skill 规则读取对应 `SKILL.md`。\n\n");
        ...
        content.append("- L0 `~/.ly-code/memory.md`\n");
        ...
        content.append("- L1 `~/.ly-code/memory/*`\n");
        ...
        content.append("- L2 `<cwd>/.ly-code/memory.md` 或项目根 `MEMORY.md`\n");
        ...
        content.append("- L3 `<cwd>/.ly-code/skills/*`\n");
