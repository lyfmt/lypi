...
public interface SessionEnginePort {
     ...
     * TODO: 打开或创建 session。
     *
     * 读取 JSONL 后构建 entry 索引和当前 leaf，不得改写历史 entry。
     */
    SessionHandle openOrCreate(String sessionId);

    /**
     * TODO: 追加 session entry。
     *
     * 这是 session 的唯一写入口，必须保持 append-only。
     */
    SessionHandle append(SessionEntry entry);

    /**
     * TODO: 查询从 leaf 到 root 的分支路径。
     *
     ...
     * TODO: 追加一条消息 entry。
     *
     * Typed helper 只能委托 append 语义，不得提供覆盖或更新能力。
     */
    SessionHandle appendMessage(AgentMessage message);

    /**
     * TODO: fork 一个新 session。
     *
