...
class JsonSubagentProcessRunnerTest {
    ...
    void rejectsStdoutWithTrailingNonJsonTokens() throws Exception {
        JsonSubagentProcessRunner runner = new JsonSubagentProcessRunner(List.of(
            "python3",
            "-c",
            "import json, sys; sys.stdin.read(); print(json.dumps({'taskName':'task','agentId':'agent_1','childSessionId':'ses_child','runId':'run_1','status':'SUCCEEDED','content':'done','finalEntryId':'entry_final'})); print('Started LyCodeApplication')"
        ));
