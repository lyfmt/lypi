...
TMP_DIR="$(mktemp -d)"
TMUX_SOCKET="lycode-tui-interaction-$$"
cleanup() {
...
printf -v PTY_COMMAND \
  'printf "SHELL_INTERACTION_SENTINEL\n"; TERM=xterm-256color java -cp %q cn.lycode.transport.tui.TuiInteractionPtyProbe %q; status=$?; printf "LYCODE_TUI_INTERACTION_EXIT=%%s\n" "$status"; IFS= read -r shell_input; printf "SHELL_INPUT=%%s\n" "$shell_input"; sleep 20' \
...
assert_substring_count "/workspace/ly-code" 1
...
if ! wait_for_full_text "LYCODE_TUI_INTERACTION_EXIT=0"; then
...
assert_substring_count "/workspace/ly-code" 0
assert_exact_count "LYCODE_TUI_INTERACTION_EXIT=0" 1
