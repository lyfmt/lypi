...
mvn -pl lycode-transport-tui -am test

if rg "cn\\.lycode\\.(agent|session|security|ai\\.provider|tool|resource|boot)\\." lycode-transport-tui/src/main/java; then
  echo "forbidden dependency reference found in lycode-transport-tui" >&2
  exit 1
fi
...
import cn.lycode.transport.tui.TerminalSession;

public final class TuiPtyProbe {
  public static void main(String[] args) throws Exception {
    try (TerminalSession ignored = TerminalSession.open()) {
      System.out.print("LYCODE_TUI_PTY_OPEN\n");
    }
    System.out.print("LYCODE_TUI_PTY_CLOSED\n");
  }
}
EOF

mvn -q -pl lycode-transport-tui dependency:build-classpath -Dmdep.outputFile="$TMP_DIR/dependency-classpath.txt"
DEPENDENCY_CLASSPATH="$(cat "$TMP_DIR/dependency-classpath.txt")"
PROBE_CLASSPATH="$ROOT/lycode-transport-tui/target/classes:$ROOT/lycode-contracts/target/classes:$DEPENDENCY_CLASSPATH"
...
bash "$ROOT/lycode-transport-tui/src/test/resources/run-tui-frame-pty.sh"
bash "$ROOT/lycode-transport-tui/src/test/resources/run-tui-interaction-pty.sh"
