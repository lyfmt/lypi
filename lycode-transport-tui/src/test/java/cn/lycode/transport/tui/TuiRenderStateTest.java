...
class TuiRenderStateTest {
    ...
    void pathLabelAbbreviatesHomeAndItsDescendants() {
        ...
        assertEquals(
            Path.of("~", "work", "ly-code").toString(),
            TuiRenderState.pathLabel(home.resolve("work/ly-code"), home)
        );
    }
    ...
    void pathLabelKeepsNormalizedAbsolutePathOutsideHome() {
        Path home = Path.of("/home/tester");
        Path cwd = Path.of("/workspace/../workspace/ly-code");

        assertEquals(
            Path.of("/workspace/ly-code").toAbsolutePath().normalize().toString(),
