...
printf -v PTY_COMMAND \
  'TERM=xterm-256color java -cp %q cn.lypi.transport.tui.TuiFramePtyProbe %q %q %q %q; status=$?; printf "\nLYPI_TUI_FRAME_EXIT=%%s\n" "$status"; sleep 30' \
...
for removed in 'SHELL_SENTINEL' 'LY-PI' 'history stable' 'stream/live row' 'status-updated'; do
