...
class DefaultBashRiskAnalyzerTest {
    ...
    void analyzeLowersStaticBashLoginCommandBeforeRiskClassification() {
        BashRiskAnalysis analysis = analyzer.analyze("bash -lc \"mvn -pl lycode-security test\"");

        assertThat(analysis.normalizedCommand()).isEqualTo("mvn -pl lycode-security test");
        assertThat(analysis.parsedCommands()).containsExactly("mvn -pl lycode-security test");
        assertThat(analysis.riskLevel()).isEqualTo(BashRiskLevel.MEDIUM);
        assertThat(analysis.staticallyKnown()).isTrue();
    }
