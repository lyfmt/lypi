...
class OpenAiCompatibleLoginRealEndToEndTest {
    private static final URI BASE_URL = URI.create(System.getProperty(
        "lycode.openai-compatible.e2e.base-url",
        "https://opencode.ai/zen/go/v1"
    ));
    private static final String CHANNEL = "compat-live";
    private static final String THINKING_MODEL = System.getProperty(
        "lycode.openai-compatible.e2e.thinking-model",
        "deepseek-v4-flash"
    );
    private static final String IMAGE_MODEL = System.getProperty(
        "lycode.openai-compatible.e2e.image-model",
        "kimi-k2.6"
    );
    private static final String TOOL_NAME = "lycode_compatibility_probe";
    ...
    void logsInAndStreamsThinkingToolContinuationAndImageChatCompletions() {
        assumeTrue(
            Boolean.getBoolean("lycode.openai-compatible.e2e"),
            "Enable with -Dlycode.openai-compatible.e2e=true"
        );
        ...
        assertManagedProperties(home.resolve(".ly-code/login-providers.properties"));
        ...
        AgentMessage toolPrompt = userMessage(
            "tool-user",
            "Call lycode_compatibility_probe exactly once with value LIVE_TOOL_OK. "
    ...
    private static String apiKey() {
        String configured = System.getProperty("lycode.openai-compatible.e2e.api-key");
        if (configured != null && !configured.isBlank()) {
            return configured;
        }
        String authEntry = System.getProperty(
            "lycode.openai-compatible.e2e.auth-entry",
            "opencode-go"
        );
