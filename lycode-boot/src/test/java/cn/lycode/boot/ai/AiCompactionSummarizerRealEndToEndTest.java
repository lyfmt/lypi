...
class AiCompactionSummarizerRealEndToEndTest {
    ...
    void compactsRealSessionContextThroughConfiguredRealProvider() throws IOException {
        assumeTrue(Boolean.getBoolean("lycode.summary.e2e"), "Enable with -Dlycode.summary.e2e=true");
    ...
    private static SessionHandle appendRealBranch(SessionManagerImpl session, RealSummarySettings settings) {
        ...
        handle = appendTurn(handle, session, 10,
            ...
            "收尾事项：更新本地 docs 工作说明但不提交 docs；运行 focused tests、lycode-agent-core tests、真实 AI E2E、mvn test；"
    ...
    private record RealSummarySettings(
        ...
        private static RealSummarySettings fromSystemProperties() {
            return new RealSummarySettings(
                requiredUri("lycode.summary.e2e.base-url"),
                required("lycode.summary.e2e.api-key"),
                System.getProperty("lycode.summary.e2e.provider", "real-summary"),
                required("lycode.summary.e2e.model"),
                ThinkingLevel.valueOf(System.getProperty("lycode.summary.e2e.thinking", "LOW").toUpperCase(java.util.Locale.ROOT))
            );
        }
