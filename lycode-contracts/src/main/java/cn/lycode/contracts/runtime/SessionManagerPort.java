...
public interface SessionManagerPort {
     ...
     */
    default cn.lycode.contracts.session.ShellState shellState() {
        throw new UnsupportedOperationException("shell state is not supported");
    }

    /**
     * Append a shell state transition to the current session branch.
     */
    default SessionHandle appendShellStateChange(cn.lycode.contracts.session.ShellState shellState) {
        throw new UnsupportedOperationException("shell state is not supported");
    }
